-- 003_schema_v2.sql
-- Snapshot of backend/schema.sql, the schema the current backend (auth.py,
-- goals.py, tasks.py, planner.py, analytics.py, ryna.py, etc.) actually
-- targets. This was applied to the live Supabase project directly via the
-- SQL editor and had never been captured as a tracked migration — this file
-- brings the migration history in line with reality. All statements are
-- idempotent (`if not exists` / `or replace`), so re-running this against
-- the live project is safe.
--
-- Supersedes 002_goalflow_v2.sql: that migration's tables (users,
-- pillar_goals, conversations) were never applied and are not used by any
-- current code. 001_goalflow_data.sql's `goalflow_data` blob table is
-- likewise orphaned (from the original single-user localStorage-sync
-- prototype) — left in place for now since it still holds legacy data, but
-- nothing in the current backend reads or writes it.
