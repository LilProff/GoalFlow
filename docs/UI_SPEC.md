...
- The step content itself, in a card, one of:
  ...
  4. **Goals** — one block per pillar selected in step 2: a labeled text field ("What does winning look like for [pillar]?"), a long-term vs. short-term toggle, and a row of goal-type chips (e.g. project-style vs. habit-style vs. metric-style) to classify that goal. The target timeline is not a fixed window — see §13 "Flexible goal timelines" for how this should work.
...
- A scripted, turn-by-turn conversation (assistant asks one question at a time, in order): name → whether the user has a clear direction or is still figuring things out → an occupation preset → 9-5 employment status → life categories to develop (multi-select) → execution pillars to focus on (multi-select) → the single biggest goal on the user's mind right now (free text) → preferred coach style (single-select from the styled list, rendered as full option cards within the chat).
...
- **Header:** an eyebrow line (today's date + the user's current streak), a large time-of-day greeting with the user's first name, a status line that changes based on today's completion ("Zero tasks done. Start executing." / "All tasks complete. Exceptional." / "N/M tasks done — keep going."), and two action buttons: **Regen Tasks** (regenerates today's AI task list; shows a spinning icon + "Generating..." while working) and **Ask Ryna** (opens the global chat panel).
- **Left column:**
...
Long-term and short-term goal tracking, structured under each pillar and each on its own timeline — see §13. Not a fixed goal-per-pillar model; a pillar can hold several goals at once (e.g. one long-term, several short-term underneath it).

- **Header:** "Goals" eyebrow + title (e.g. "Your goals, structured by timeline"), an **AI Planner** button (opens the page's own embedded Goal-Planner chat panel described below — not the global Ryna panel), and a **+ New Goal** button.
...
## 12. Flexible goal timelines (design note, not yet built)

The product direction is shifting away from a single fixed "90-day sprint" applied to every goal. Design for this model instead:

- Every goal is tagged **long-term** or **short-term**, not forced into one universal window. A long-term goal (e.g. a career or life-level ambition) can have short-term goals nested under it as the concrete steps toward it.
- Each goal gets a **prospective timeline** — an estimated target date/duration the system proposes based on the goal's scope and the user's stated pace — rather than everyone defaulting to the same 90 days. This should read in the UI as a flexible duration/date, not a hardcoded countdown.
- That timeline is meant to **adapt**: as the user executes (or falls behind), the system nudges the prospective date forward or back rather than leaving a fixed deadline the user just blows past. Wherever a goal shows a "days left" style readout, design it to also accommodate a state like "on track," "behind — timeline adjusted," or "ahead of pace," not only a countdown number.
- A goal card/detail view should be able to show its place in a hierarchy (which long-term goal a short-term one ladders up to) — this doesn't exist as a flat list-of-cards model.

Design the Goals screen (and anywhere else a goal timeline appears — Dashboard, Onboarding, Analytics) with this flexible/adaptive model in mind rather than the literal "90-Day Sprint" framing described elsewhere in this document, which reflects the current build and is being phased out.
